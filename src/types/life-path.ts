...
// ── Week Plan (todo-style weekly management) ─────────────────────────────────

/**
 * A single actionable task within a week plan.
 * Tasks are grouped by `goalId` so users see their week organised by
 * the life goals they are working toward.
 */
export interface WeekTask {
  id: string;
  /** Which UserGoal this task belongs to. Empty string = standalone. */
  goalId: string;
  /** Task text, user-editable. */
  text: string;
  /** Whether the task has been completed. */
  done: boolean;
  /** ISO datetime when the task was marked done, or null. */
  completedAt: string | null;
  /** ISO datetime when the task was created. */
  createdAt: string;
  /** "ai" if generated by the mentor, "user" if manually added. */
  source: "ai" | "user";
}

/**
 * A full week plan — the focus theme, AI-surfaced trap warning, and
 * a flat list of tasks (grouped in UI by goalId).
 */
export interface WeekPlan {
  /** ISO week key, e.g. "2024-W16" */
  weekKey: string;
  /** User-editable focus theme for the week. */
  focus: string;
  /** AI-generated warning / pitfall to watch out for this week. */
  trap: string;
  /** Whether the user has dismissed the trap warning. */
  trapDismissed: boolean;
  /** All tasks for the week, grouped in UI by goalId. */
  tasks: WeekTask[];
  /** ISO datetime when the plan was first created. */
  createdAt: string;
  /** ISO datetime of the most recent AI generation. */
